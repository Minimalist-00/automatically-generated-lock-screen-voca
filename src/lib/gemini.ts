...
### 例文とシーン生成のルール
- 各候補の "scene" と "example" は【必ず同じシチュエーション】に基づいてください。sceneが「ゲーム中に仲間を褒める」なら、exampleもそのゲーム中のシーンでの例文にしてください。sceneとexampleがちぐはぐにならないよう、絶対に一致させてください。
- sceneは使うシーンと感情・ニュアンスを【最大30文字以内】で超一言で。状況説明は極力省き、「いやそれな！」「まじかよ！」など、Neoのバイブスに合う日本語の口語表現メインに。長文禁止。
...
- "candidates": 異なるシチュエーションやニュアンスを持つ3つの例文候補の配列。各候補は "scene" と "example" を含みます。
