...
export default function GlassmorphicCard({ words }: GlassmorphicCardProps) {
  return (
    <div className="relative overflow-hidden rounded-3xl border-2 border-[#D1EAE5]/60 bg-[var(--card-bg)]/95 p-5 shadow-[0_8px_24px_rgba(165,207,201,0.25)] backdrop-blur-md flex flex-col gap-6">
      {words.slice(0, 3).map((w, index) => (
        <div key={w.id || index} className="flex flex-col gap-3">
          
          <div className="flex flex-col gap-1">
            <span className="text-base font-bold text-[var(--accent)] leading-none">{w.word}</span>
