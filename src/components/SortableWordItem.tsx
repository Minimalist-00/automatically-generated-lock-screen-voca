...
export default function SortableWordItem({
  ...
  return (
    <div 
      ref={setNodeRef} 
      style={style} 
      className={`cute-card p-3 bg-white hover:translate-x-[-1px] hover:translate-y-[-1px] hover:shadow-[3px_3px_0px_0px_#2D3748] transition-all flex flex-col gap-1.5 ${isSelected ? 'border-2 border-[#2B6CB0] bg-[#EBF8FF]' : ''} ${isDragging ? 'shadow-[5px_5px_0px_0px_#2B6CB0] border-[#2B6CB0]' : ''}`}
    >
      {isEditing ? (
